...
we compressed the full sets of features to a smaller set that capture the overarching patterns of each aberration type.
...
This is the standard first step for clustering based on a single aberration type.
...
Either a correlation matrix or a consensus correlation matrix can be identically used to create subtypes.
