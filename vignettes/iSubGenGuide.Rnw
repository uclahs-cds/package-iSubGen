...
based on the relationships between molecular aberration profiles (ex. mRNA, single nucleotide variants, copy number alterations, \textit{etc.})
...
we compressed full set of features to a smaller set that capture the overarching patterns of each aberration type.
...
An autoencoder is a neural net which is trained to match th output to the input it was given. 
...
\begin{figure}[H]
	\centering
		\includegraphics[width=9.8cm]{Figures/iSubGen_figure_methodolody_autoencoders.png}
...
Here we create 2 compressed features for the CNA feature set and 1 compressed feature for each of the SNV and methylation feature sets.
...
\begin{figure}[H]
	\centering
		\includegraphics[width=9.8cm]{Figures/iSubGen_figure_methodolody_correlations.png}
