...
\begin{figure}[H]
	\centering
		\includegraphics[width=9.5cm]{Figures/iSubGen_figure_methodolody_autoencoders.pdf}
...
\begin{figure}[H]
	\centering
		\includegraphics[width=9.5cm]{Figures/iSubGen_figure_methodolody_correlations.pdf}
...
\textbf{Activation function:} defines the output of neural network nodes.

\textbf{Autoencoder:} an unsupervised approach for neural networks where the neural network tries to reconstruct its input. \\

\textbf{CNA:} copy number alteration. A change in the number of copies of a segment of DNA. 
...
\textbf{Consensus clustering:} a process of iteratively clustering to essentially bootstrap the clustering result. \url{https://bioconductor.org/packages/release/bioc/html/ConsensusClusterPlus.html}

\textbf{Methylation:} a DNA modification where methyl groups of are added to cytosines in DNA to help regulate DNA transcription. \\

\textbf{Molecular aberration type (or aberration type):} encapsulates the different changes that occur the DNA, RNA and other biomolecules during cancer. For example: CNAs, SNVs, methylation, mRNA abundance, miRNA abundance.\\

\textbf{SNV:} single nucleotide variant. a nucleotide base (A,C,G,T) change in DNA.\\

\textbf{keras R package:} \url{https://cran.r-project.org/package=keras} \\ 
